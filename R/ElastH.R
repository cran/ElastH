...
#' da função \code{\link{criar.dlm}}. O resultado da função é uma lista com
#' várias informações, sendo que a mais importante é a variável
#' \code{comp}, que detalha os valores dos componentes. Vide seção
#' \code{\link{criar.dlm}} para mais informações sobre as entradas, as
#' saídas e o funcionamento desta função.
#' 
#' A função \code{\link{todas.dlms}} permite estimar com facilidade um conjunto
...
#' função ver seção \code{\link{todas.dlms}}, para mais informações sobre os
#' componentes não observados ver a seção detalhes da função
#' \code{\link{criar.dlm}}.
#'
#' A função \code{\link{calcular.elasticidades}} faz uso de
#' \code{\link{todas.dlms}}, para calcular as elasticidades de todo o conjunto
...
#' funções todas.dlms ou calcular.elasticidades. Depois de estimados os modelos
...
#' \code{nt} da lista resultante da função \code{\link{criar.dlm}}. As melhores
